...
def is_fair(sequence):
    ...
    while len(possibilities) > 0:
        thing_to = possibilities.pop()
        for card in range(len(thing_to)):
            if last_card == -1:
                last_card = thing_to[card]
            else:
                if thing_to[card] == last_card:
                    who_not_turn = 0 if who_turn == 1 else 1
                    remove_amount = player_chance[who_turn] / 2
                    player_chance[who_not_turn] += remove_amount
                    player_chance[who_turn] -= remove_amount
                    possibilities.append(thing_to[card:])
                else:
                    who_not_turn = 0 if who_turn == 1 else 1
                    remove_amount = player_chance[who_not_turn] / 2
                    player_chance[who_turn] += remove_amount
                    player_chance[who_not_turn] -= remove_amount
    '''

    ...
    for i in spi:
        ...
        for j in i:
            if j < 10:
                ...
                temp.append(str(j))
        print(temp)
#Test function so like not used for realsies
...
def generate_sprial(size,clockwise=True):
    ...
    try:
        ...
        if clockwise:
            number_to_yeah = cool_spiral[1][size-2]+2
            cool_spiral[1][size-1] = number_to_yeah-1
            cur_dir = 1
            current_postion = [2, size-1]
            allowed_length = size-2
        else:
            number_to_yeah = cool_spiral[size-2][size-2]+2
            current_postion = [size-3, size-1]
            cur_dir = 3
            cool_spiral[size-2][size-1] = number_to_yeah-1
            allowed_length = size-2
    except:
        cool_spiral = []
        for i in range(size):

...
def zero_destroyer(strng): #see how many 0s any move would remove
    ...
    if strng != "0":
        if len(strng) == 1:
            ...
            for index in range(1,len(strng)):
                if strng[index] == "1":
                    break
                max_leading = index
    return max_leading
...
def game_time(start):
    ...
    while True:
        ...
        if sum_with_strs(start) == 0:
            break
        if OnesTurn:
            best_descruction = (0,0)
            for index, i in enumerate(start):
                destr = zero_destroyer(i)
                if destr >= best_descruction[0]:
                    best_descruction = (destr, index)
            #how_many,which sub index, which_index
            choice = best_descruction[1]
            choice2 = 0#best_descruction[1]
        else:
            best_descruction = (-1,0)
            for index, i in enumerate(start):
